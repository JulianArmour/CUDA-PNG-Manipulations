...
__global__ void pool(unsigned char output_image[], const unsigned char image[], unsigned int width_out, unsigned int height_out) {
  ...
  int pixel_out = blockDim.x * blockDim.z * blockIdx.x + blockDim.z * threadIdx.x;
  int index_out = pixel_out + threadIdx.z;  // index = pixel + channel
  ...
  int pixel_in = width_out * 16 * y_out + x_out * 8;  

  if (y_out >= height_out) return;
...
int main(int argc, char** argv) {
  ...
  cudaMemcpy((void*)d_old_image, (void*)old_image, width * height * 4, cudaMemcpyHostToDevice);
